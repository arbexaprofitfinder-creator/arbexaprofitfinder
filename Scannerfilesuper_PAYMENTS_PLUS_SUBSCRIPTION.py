...
# --- SUBSCRIPTION helper ---
def _sb_upsert_subscription_for_user(*, user_id: str, plan: str | None = None, status: str = "active", note: str | None = None):
    """
    Create/refresh a subscription row for `user_id`.
    If a row exists for this user, we upsert it (1 row per user).
    Status is "active" by default; caller can pass "pending".
    Start/end are computed from `plan` only when status == "active".
    """
    import datetime as _dt
    _days_map = {"weekly":7, "monthly":30, "3m":90, "6m":180, "yearly":365, "3y":1095}
    _days = _days_map.get(str(plan), 30)

    if status == "active":
        _start = _dt.datetime.utcnow()
        _end = _start + _dt.timedelta(days=_days)
        row = {"user_id": user_id, "status": "active",
               "start_at": _start.strftime("%Y-%m-%dT%H:%M:%SZ"),
               "end_at": _end.strftime("%Y-%m-%dT%H:%M:%SZ"),
               "note": note}
    else:
        row = {"user_id": user_id, "status": status,
               "start_at": None, "end_at": None, "note": note}
    try:
        _sb_upsert("SUBSCRIPTION", [row], on_conflict="user_id")
    except Exception as _e:
        try: _sb_post_rows("SUBSCRIPTION", [row])
        except Exception as _e2: print("[subscription] post error:", _e2)
...
def pro_checkout(plan: str = 'monthly', username: str | None = None):
    ...
    # Pending SUBSCRIPTION upsert (activates later on payment confirmation)
    try:
        if supa_uid:
            _sb_upsert_subscription_for_user(user_id=supa_uid, plan=str(plan), status="pending",
                                             note=f"awaiting payment: {sr_invoice_id or provider_invoice_id}")
    except Exception as _se:
        print("[subscription] pending upsert warn:", _se)
