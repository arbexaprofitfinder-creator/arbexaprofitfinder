...
# ----- SETTINGS API (save + fetch) -----
from typing import Optional as _Optional, List as _List

class SettingsIn(BaseModel):
    edge_pct_min: float = 1.0
    volume_24h_usd_min: float = 0.0
    liquidity_min: float = 0.0
    pair_filter: _Optional[str] = None
    trade_size_min_usd: _Optional[float] = None
    trade_size_max_usd: _Optional[float] = None
    sound_on: bool = True
    exchanges_enabled: _List[str] = []

def _default_settings_dict():
    return {
        "edge_pct_min": 1.0,
        "volume_24h_usd_min": 0.0,
        "liquidity_min": 0.0,
        "pair_filter": None,
        "trade_size_min_usd": None,
        "trade_size_max_usd": None,
        "sound_on": True,
        "exchanges_enabled": [],
    }

@app.post("/api/settings/save", response_class=JSONResponse)
def save_settings(data: SettingsIn, request: Request, db: Session = Depends(get_db)):
    u = current_user_from_auth_header(request, db)
    if not u:
        raise HTTPException(status_code=401, detail="Not authenticated")
    try:
        supa_uid = _sb_get_profile_id(email=u.email)
        if supa_uid:
            row = {
                **data.dict(),
                "user_id": supa_uid,
                "updated_at": dt.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"),
            }
            try:
            ...
            try:
                _sb_post_rows("SETTINGS", [row])  # direct
            except Exception as _e:
                print("[supabase] SETTINGS direct post error:", _e)
        else:
            print("[supabase] SETTINGS skip (no PROFILES id)")
    except Exception as _e:
        print("[supabase] SETTINGS mirror error:", _e)
    return {"ok": True}
...
def get_settings(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        supa_uid = _sb_get_profile_id(email=u.email)
        if not supa_uid or not SUPABASE_URL or not SUPABASE_SERVICE_ROLE_KEY:
            return JSONResponse({**defaults, "user_id": None})
        base = SUPABASE_URL.rstrip("/") + "/rest/v1/SETTINGS"
        params = f"?select=*&user_id=eq.{supa_uid}&order=updated_at.desc.nullslast&limit=1"
        hdrs = {
            "apikey": SUPABASE_SERVICE_ROLE_KEY,
            "Authorization": f"Bearer {SUPABASE_SERVICE_ROLE_KEY}",
            "Accept": "application/json",
        }
        r = requests.get(base + params, headers=hdrs, timeout=8)
        if r.status_code == 200 and r.headers.get("content-type","").startswith("application/json"):
            ...
            if isinstance(arr, list) and arr:
                ...
                for k in defaults.keys():
                    if k in rec:
                        out[k] = rec[k]
                return JSONResponse({**out, "user_id": supa_uid})
        return JSONResponse({**defaults, "user_id": supa_uid})
    except Exception as _e:
        print("[supabase] SETTINGS fetch error:", _e)
        return JSONResponse({**defaults, "user_id": None})
...
(function(){
  try{
    const ids={minEdge:'#minEdge',maxEdge:'#maxEdge',minVol:'#minVol',minLiq:'#minLiq',pair:'#q',tsMin:'#tsMin',tsMax:'#tsMax',sound:'#soundToggle'};
    function tok(){try{return localStorage.getItem('arbexa_token')||localStorage.getItem('token')||localStorage.getItem('access_token')}catch(_){return null}}
    const debounce=(f,m)=>{let t;return(...a)=>{clearTimeout(t);t=setTimeout(()=>f(...a),m);}};
    async function save(p){const t=tok(); if(!t) return; try{ await fetch('/api/settings/save',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+t},body:JSON.stringify(p)});}catch(e){console.warn('settings save error',e)}}
    const go=debounce(save,600);
    function num(sel,d){const el=document.querySelector(sel); if(!el) return d; const v=parseFloat(el.value); return Number.isFinite(v)?v:d}
    function str(sel){const el=document.querySelector(sel); return el ? (el.value||null) : null}
    function payload(){
      const sbtn=document.querySelector(ids.sound);
      const soundOn = sbtn ? String(sbtn.getAttribute('aria-pressed')||'true').toLowerCase()!=='false' : true;
      return {edge_pct_min:num(ids.minEdge,1), volume_24h_usd_min:num(ids.minVol,0), liquidity_min:num(ids.minLiq,0),
              pair_filter:str(ids.pair), trade_size_min_usd:num(ids.tsMin,null), trade_size_max_usd:num(ids.tsMax,null),
              sound_on:soundOn, exchanges_enabled:[]};
    }
    function hook(sel,ev){const el=document.querySelector(sel); if(!el) return; el.addEventListener(ev,()=>go(payload()))}
    hook(ids.minEdge,'input'); hook(ids.maxEdge,'input'); hook(ids.minVol,'input'); hook(ids.minLiq,'input');
    hook(ids.pair,'input'); hook(ids.tsMin,'input'); hook(ids.tsMax,'input');
    const s=document.querySelector(ids.sound); if(s){ s.addEventListener('click', ()=>go(payload())); }
    setTimeout(()=>go(payload()), 1000);
  }catch(e){ console.warn('autosave init error', e); }
})();
</script>
