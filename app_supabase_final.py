...
import os as _os
try:
    from supabase import create_client as _create_client, Client as _Client  # type: ignore
    _URL = _os.getenv("SUPABASE_URL")
    _KEY = _os.getenv("SUPABASE_ANON_KEY")
    supabase: "_Client" = None  # type: ignore
    if _URL and _KEY:
        supabase = _create_client(_URL, _KEY)
        print("[supabase] client ready")
    else:
        supabase = None
        print("[supabase] missing SUPABASE_URL / SUPABASE_ANON_KEY — skipping")
except Exception as _e:
    supabase = None  # type: ignore
    print("[supabase] init failed:", _e)
# === end Supabase setup ===
import requests
...
def signup(data: SignupIn, db: Session = Depends(get_db)):
    ...
    try:
        \1
    # Supabase: create/ensure profile row
    try:
        save_profile_to_sb(email, username)
    except Exception as _e:
        print('[supabase] profile insert error:', _e)
except Exception:
        db.rollback()
...
def _sb_ready() -> bool:
    return bool('supabase' in globals() and supabase)

def sb_safe_insert(table: str, data: dict):
    try:
        if _sb_ready():
            supabase.table(table).insert(data).execute()
    except Exception as e:
        print(f"[supabase] insert failed into {table}: {e}")

def save_profile_to_sb(email: str, username: str):
    # profiles: email (unique, nullable), username (unique, nullable)
    if not (email or username):
        return
    data = {"email": email or None, "username": username or None}
    sb_safe_insert("profiles", data)  # lowercase table name

def save_opportunity_to_sb(row: dict):
    # opportunities mapping according to your schema
    try:
        ...
        data = {
            ...
            "exchange_buy": row.get("buy_ex"),
            "exchange_sell": row.get("sell_ex"),
            "price_buy": row.get("buy_ask"),
            "price_sell": row.get("sell_bid"),
            "spread_pct": row.get("edge"),
            "volume_24h_usd": (row.get("qv_buy") or row.get("qv_sell")),
            "liquidity": row.get("liquidity"),
            "est_profit_usd": row.get("example_profit_1000"),
            "raw": row,  # jsonb
            "user_id": None,  # nullable FK
        }
        sb_safe_insert("opportunities", data)  # lowercase table name
    except Exception as e:
        print("[supabase] save_opportunity_to_sb error:", e)
    ...
    for sym in candidates:
        ...
        with lock:
            ...
            if (not prev) or row["edge"] > prev["edge"]:
                try:
                    save_opportunity_to_sb(row)
                except Exception as _e:
                    print('[supabase] opp insert error:', _e)
