...
try:
    ...
    if _URL and _KEY:
        ...
        supabase = None
        print("[supabase] missing SUPABASE_URL / key — skipping")
...
def signup(data: SignupIn, db: Session = Depends(get_db)):
    ...
    # Supabase: create/ensure profile row (added)
    try:
...
def save_profile_to_sb(email: str, username: str):
    ...
    sb_safe_insert("profiles", data)  # change to PROFILES if needed
...
def save_opportunity_to_sb(row: dict):
    try:
        ...
        sb_safe_insert("opportunities", data)  # change to OPPORTUNITIES if needed
    except Exception as e:
    ...
    for sym in candidates:
        ...
        with lock:
            ...
            if (not prev) or row["edge"] > prev["edge"]:
                # Supabase: store new/better opportunity (added)
                try:
