...
def pro_checkout(
    ...
    # Resolve Supabase user_id (PROFILES.id)
    supa_uid = None
    try:
        u = current_user_from_auth_header(request, db)
        if u:
    ...
    if not supa_uid and username:
        try:
            supa_uid = _sb_get_or_create_profile_id(email=None, username=username)
        except Exception as _e:
    ...
    try:
        r = requests.post("https://api.cryptocloud.plus/v2/invoice/create", json=payload, headers=headers, timeout=20)
    except Exception as e:
