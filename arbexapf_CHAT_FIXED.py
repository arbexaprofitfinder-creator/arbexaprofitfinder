...
def _sb_enqueue_and_post(table: str, row: dict):
    """Enqueue and also try immediate insert (same behavior as opportunities)."""
    try:
        _sb_enqueue(table, row)
        try:
            _ok = _sb_post_rows(table, [row])
            print("[supabase] direct insert", table, "->", _ok)
        except Exception as _e:
            print("[supabase] direct insert error", table, _e)
    except Exception as _e:
        print("[supabase] enqueue/post error:", _e)

def _sb_get_profile_id(email: str | None = None, username: str | None = None):
    """Resolve Supabase PROFILES.id from email/username."""
    if not SUPABASE_URL or not SUPABASE_SERVICE_ROLE_KEY:
        return None
    try:
        from urllib.parse import quote
        if email:
            filt = "email=eq." + quote(email, safe="")
        elif username:
            filt = "username=eq." + quote(username, safe="")
        else:
            return None
        url = f"{SUPABASE_URL}/rest/v1/PROFILES?select=id&limit=1&" + filt
        hdrs = {
            "apikey": SUPABASE_SERVICE_ROLE_KEY,
            "Authorization": f"Bearer {SUPABASE_SERVICE_ROLE_KEY}",
            "Content-Type": "application/json",
        ...
        if r.status_code == 200:
            ...
            print("[supabase] get profile id HTTP", r.status_code, r.text[:200])
    except Exception as e:
        print("[supabase] get profile id error:", type(e).__name__, e)
    return None
...
def post_chat_message(data: ChatIn, request: Request, db: Session = Depends(get_db)):
    ...
    # === Mirror to Supabase CHAT (requires PROFILES row) ===
    try:
        supa_uid = _sb_get_profile_id(email=u.email if hasattr(u, 'email') else None,
                                      username=u.username if hasattr(u, 'username') else None)
        if supa_uid:
            _sb_enqueue_and_post("CHAT", {
                ...
                "reply_to": None,
                "is_system": False,
                "meta": {"local_user_id": getattr(u, 'id', None), "local_msg_id": getattr(msg, 'id', None)}
            })
        else:
            print("[supabase] CHAT skip (no PROFILES id for user)")
    except Exception as _e:
