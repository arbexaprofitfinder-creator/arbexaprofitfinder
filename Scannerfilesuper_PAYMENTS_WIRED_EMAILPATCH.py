...
def pro_checkout(plan: str = 'monthly', username: str | None = None):
    ...
    # Resolve user_id and user_email
    supa_uid = None
    user_email = None
    try:
    ...
    if u:
        ...
        if not supa_uid and user_email:
            try:
                supa_uid = _sb_get_or_create_profile_id(email=user_email, username=getattr(u, "username", None))
            except Exception as _e:
                print("[payments] profile resolve error:", _e)

    if not user_email and email_q:
        user_email = email_q.strip().lower()

    if not supa_uid and user_email:
        try:
            uname = None
            try: uname = request.query_params.get("username")
            except Exception: pass
            supa_uid = _sb_get_or_create_profile_id(email=user_email, username=uname or "anon")
        except Exception as _e:
    ...
    safe_uname = (locals().get("username") or (getattr(u, "username", None) if u else None) or "anon")
    ...
    try:
        r = requests.post("https://api.cryptocloud.plus/v2/invoice/create", json=payload, headers=headers, timeout=(5,45))
    except Exception as e:
        try:
            ...
            print("[payments] pending enqueue failed:", _e2)
        return HTMLResponse(f"CryptoCloud is slow right now. We saved your attempt; please try again. Error: {e}", status_code=504)
