...
def try_build_opps_incremental():
    ...
    for sym in candidates:
        ...
        if not ob_sell:
            ob_sell = {"asks": [], "bids": []}
        qv_buy = snapshots[ex_buy].get(sym, {
            "ob_buy_asks": ob_buy.get("asks", []) if isinstance(ob_buy, dict) else [],
            "ob_buy_bids": ob_buy.get("bids", []) if isinstance(ob_buy, dict) else [],
            "ob_sell_asks": ob_sell.get("asks", []) if isinstance(ob_sell, dict) else [],
            "ob_sell_bids": ob_sell.get("bids", []) if isinstance(ob_sell, dict) else [],
            "ob_loading": ob_loading,
            "status_note": ("orderbook loading" if ob_loading else None),
        }).get("qvol", 0.0)
        qv_sell = snapshots[ex_sell].get(sym, {}).get("qvol", 0.0)
        ls = 0 if ob_loading else liquidity_score_v2(qv_buy, qv_sell, ob_buy["asks"], ob_sell["bids"])
        lo, hi = (0.0, 0.0) if ob_loading else suggested_trade_range_usd(qv_buy, qv_sell, ob_buy["asks"], ob_sell["bids"])
        row = {
