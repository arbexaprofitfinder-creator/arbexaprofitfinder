...
def save_settings(data: SettingsIn, request: Request, background: BackgroundTasks, db: Session = Depends(get_db)):
    u = current_user_from_auth_header(request, db)
    if not u:
        raise HTTPException(status_code=401, detail="Not authenticated")
    try:
        ...
        if supa_uid:
            ...
            try:
                background.add_task(_sb_post_rows, "SETTINGS", [row])
            except Exception as _e:
                print("[supabase] SETTINGS bg post error:", _e)
        else:
