...
<section class="sec grid fade">
  ...
  <div class="box">
    ...
    <div class="socials">
      <a class="btn" target="_blank" rel="noopener" href="https://x.com/arbexascanner?s=21">Twitter / X</a>
      <a class="btn" target="_blank" rel="noopener" href="https://www.youtube.com/@ArbexaProfitFinder">YouTube</a>
      <a class="btn" target="_blank" rel="noopener" href="https://www.tiktok.com/@arbexaprofitfinder">TikTok</a>
      <a class="btn" target="_blank" rel="noopener" href="https://t.me/Arbexaprofitfinderchannel">Telegram Channel</a>
      <a class="btn" target="_blank" rel="noopener" href="https://t.me/Arbexaprofitfinderchat">Telegram Chat</a>
      <a class="btn" target="_blank" rel="noopener" href="https://t.me/ArbexaProfitFinderSupport">Telegram Support</a>
    </div>
  </div>
</section>
...
(async ()=>{
  try{
    ...
    wrap.innerHTML = arr.map(ex=>{
      ...
      return `<div class="logo"><img alt="${ex}" src="${src}" onerror="this.style.display='none'"></div>`;
    }).join('');
  }catch(_){}
})();
</script>
...
<button id="chatFab" class="chatfab" title="Open chat" aria-label="Open chat">
  <span class="dot">💬</span>
</button>
<script>
(function(){
  ...
  function pos(){ const r=fab.getBoundingClientRect(); left=r.left; top=r.top; }
  function down(e){ dragging=true; pos(); sx=(e.touches?e.touches[0].clientX:e.clientX); sy=(e.touches?e.touches[0].clientY:e.clientY); e.preventDefault(); }
  ...
  fab.addEventListener('click', (ev)=>{ if(dragging){ dragging=false; return; } try{ const t=localStorage.getItem('arbexa_token'); if(!t){ location.href='/login'; return; } }catch(_){ } location.href='/chat'; });
})();
</script>
</body></html>
"""
...
def list_chat_messages(request: Request, db: Session = Depends(get_db)):
    ...
    if not u:
        raise HTTPException(status_code=401, detail="Not authenticated")
    rows = db.execute(select(Message).order_by(Message.created_at.asc()).limit(300)).scalars().all()
    out = []
    for r in rows:
        out.append({
            "id": r.id,
...
def post_chat_message(data: ChatIn, request: Request, db: Session = Depends(get_db)):
    u = current_user_from_auth_header(request, db)
    if not u:
        raise HTTPException(status_code=401, detail="Not authenticated")
    ...
    uname = (u.username or "") or re.sub(r"[^a-z0-9]", "", (u.email or "").split("@",1)[0].lower()) or "user"
...
async function loadMessages(){
  ...
  const j = await r.json().catch(()=>({}));
  if(!r.ok || !j.ok){ throw new Error(j.detail||('Failed to load messages ('+r.status+')')); }
  const list = $('#list');
  list.innerHTML = (j.messages||[]).map(m => {
  ...
  (j.messages||[]).forEach((m,i)=>{ els[i].textContent = m.text || ''; });
