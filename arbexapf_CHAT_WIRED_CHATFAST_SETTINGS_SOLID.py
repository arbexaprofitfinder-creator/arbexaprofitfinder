...
# ---------- SETTINGS endpoints (enqueue + direct) ----------
...
def save_settings(data: SettingsIn, request: Request, db: Session = Depends(get_db)):
    ...
    if not supa_uid:
        raise HTTPException(status_code=500, detail="Could not resolve Supabase user_id")
    ...
    try: _sb_enqueue("SETTINGS", row)
    except Exception as _e: print("[supabase] SETTINGS enqueue error:", _e)
    try: _sb_post_rows("SETTINGS", [row])
    except Exception as _e: print("[supabase] SETTINGS direct post error:", _e)
    ...
    try:
        ...
        if r.status_code == 200 and r.headers.get("content-type","").startswith("application/json"):
            ...
            if isinstance(arr, list) and arr:
                rec = arr[0]
                out = {
                    "edge_pct_min": rec.get("edge_pct_min", 1.0),
                    "volume_24h_usd_min": rec.get("volume_24h_usd_min", 0.0),
                    "liquidity_min": rec.get("liquidity_min", 0.0),
                    "pair_filter": rec.get("pair_filter"),
                    "trade_size_min_usd": rec.get("trade_size_min_usd"),
                    "trade_size_max_usd": rec.get("trade_size_max_usd"),
                    "sound_on": rec.get("sound_on", True),
                    "exchanges_enabled": rec.get("exchanges_enabled", []),
                    "user_id": supa_uid
                }
