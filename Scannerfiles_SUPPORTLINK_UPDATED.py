    ...
    for sym in candidates:
        ...
        with lock:
            ...
            if (not prev) or row["edge"] > prev["edge"]:
                opps_cache[sym] = row
                # Save to Supabase only when this symbol is new (avoid spamming on every improvement)
                try:
                    if prev is None:
                        _base, _quote = (sym.split('/') + [None, None])[:2]
                        _op = {
                            "pair": sym,
                            "base": _base,
                            "quote": _quote,
                            ...
                            "price_buy": float(ask) if ask is not None else None,
                            "price_sell": float(bid) if bid is not None else None,
                            "spread_pct": float(e) if e is not None else None,
                            "volume_24h_usd": float(min(qv_buy or 0.0, qv_sell or 0.0)) if (qv_buy or qv_sell) else None,
                            "liquidity": float(ls) if ls is not None else None,
                            "est_profit_usd": float(max(0.0, 1000.0 * e / 100.0)) if e is not None else None,
                            "raw": {
                                "ob_buy_asks": ob_buy.get("asks"),
                                "ob_sell_bids": ob_sell.get("bids"),
                                "mid": row.get("price"),
                                "qv_buy": qv_buy, "qv_sell": qv_sell
                            }
                        }
                        save_opportunity_to_supabase(_op)
                except Exception as _e:
                    print("[supabase] save-op error:", _e)
