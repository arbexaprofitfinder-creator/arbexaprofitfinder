...
.scrim{position:fixed; inset:0; background:rgba(8,12,22,.6); display:none; z-index:60;}
...
(function(){
  ...
  function openDrawer(){ scrim.classList.add('show'); drawer.classList.add('show'); drawer.classList.add('open'); drawer.setAttribute('aria-hidden','false'); }
  function closeDrawer(){ scrim.classList.remove('show'); drawer.classList.remove('show'); drawer.classList.remove('open'); drawer.setAttribute('aria-hidden','true'); }
  openBtn && openBtn.addEventListener('click', openDrawer);
  closeBtn && closeBtn.addEventListener('click', closeDrawer);
  scrim && scrim.addEventListener('click', closeDrawer);

  // Robust tap binding for mobile (iOS/Android) + desktop
  ...
  document.getElementById('drLogout')?.addEventListener('click', ()=>{ try{ document.getElementById('btnLogout')?.click();
  // Extra mobile-safe bindings
  wireTap(document.getElementById('drSettings'), ()=>{ openDetails('settingsDD'); closeDrawer(); });
  wireTap(document.getElementById('drSocials'), ()=>{ openDetails('socialsDD'); closeDrawer(); });
  wireTap(document.getElementById('drTnc'), ()=>{ openDetails('tncDD'); closeDrawer(); });
  wireTap(document.getElementById('drProfile'), ()=>{ openDetails('profileDD'); closeDrawer(); });
  wireTap(document.getElementById('drMsg'), ()=>{ openDetails('msgDD'); closeDrawer(); });
  wireTap(document.getElementById('drLogout'), ()=>{ try{ logoutUser && logoutUser(); }catch(_){ location.href='/logout'; } closeDrawer(); });
 }catch(_){} closeDrawer(); });
})();
</script>
