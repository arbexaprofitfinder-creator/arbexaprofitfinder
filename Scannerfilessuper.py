...
@media (max-width: 900px) {
  ...
  .header, .app-header, header { position: sticky; top: 0; z-index: 1000; background: inherit; }
  /* Drawer pattern */
  .drawer { position: fixed; inset: 0 0 0 auto; width: min(88%, 360px); max-width: 92vw;
            background: #0f0f0f; border-left: 1px solid #1b1b1b; transform: translateX(100%);
            transition: transform .25s ease; z-index: 1200; overflow:auto; }
  .drawer.open, .drawer.show { transform: none; }
  /* Basic container padding */
  .container, .content, .main, .app-main { padding: 12px; }
  /* Grid to single column */
  .grid, .columns, .row { display: block; }
  /* Common right column (order book/trades) stacks */
...
document.addEventListener('click', function(e) {
  ...
  const burger = document.querySelector('.burger, .app-burger, [data-toggle="drawer"]');
  const closeBtn = document.querySelector('.drawer-close, [data-close="drawer"]');
  if (!drawer) return;
  if (e.target.closest('.burger, .app-burger, [data-toggle="drawer"]')) { drawer.classList.add('open'); }
  if (e.target.closest('.drawer-close, [data-close="drawer"]') || (!e.target.closest('.drawer') && !e.target.closest('.burger') && !e.target.closest('[data-toggle="drawer"]'))) {
...
/* === Mobile drawer & icon-only menu === */
.drawer-btn{display:none; width:40px; height:40px; border-radius:12px; border:1px solid #23345f; background:#0e1a35; color:#e7eefc; font-weight:900; display:none; align-items:center; justify-content:center; box-shadow:0 1px 0 rgba(0,0,0,.2) inset;}
.drawer-btn:active{transform:scale(.98)}
.scrim{position:fixed; inset:0; background:rgba(8,12,22,.6); display:none; z-index:60; pointer-events:none;}
.scrim.show{display:block; pointer-events:auto}
.drawer{position:fixed; top:0; right:0; bottom:0; width:min(90vw,380px); background:#0f1730; border-left:1px solid #23345f; transform:translateX(100%); transition:transform .18s ease; z-index:70; display:flex; flex-direction:column;}
.drawer.show, .drawer.open{transform:translateX(0)}
.drawer .dh{display:flex; align-items:center; justify-content:space-between; gap:8px; padding:14px; border-bottom:1px solid #1a2547}
.drawer .list{padding:8px 10px; display:grid; gap:4px}
.drawer .item{display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:12px; border:1px solid #1f2a44; background:#0e1a35; color:#e7eefc; font-weight:800; cursor:pointer}
.drawer .item:active{transform:scale(.98)}
.drawer .logout{color:#ff6b6b; border-color:#3b1f20; background:#221112}
@media (max-width: 600px){
  .menu-anchor details > summary{ display:none; }   /* hide text Menu / Profile dropdowns */
  .auth-anchor{ display:none; }
...
<aside id="drawer" class="drawer" aria-hidden="true" aria-label="Mobile menu">
  ...
  <div class="list">
    <div id="drProfile" class="item">👤 Profile</div>
    <div id="drSettings" class="item">⚙️ Settings</div>
    <div id="drSocials" class="item">👥 Socials</div>
    <div id="drTnc" class="item">📄 Terms & Conditions</div>
    <div id="drMsg" class="item">💬 Message</div>
    <div id="drLogout" class="item logout">🚪 Log out</div>
  </div>
</aside>
<script>
(function(){
  ...
  function openDrawer(){ scrim.classList.add('show'); drawer.classList.add('open'); drawer.setAttribute('aria-hidden','false'); }
  function closeDrawer(){ scrim.classList.remove('show'); drawer.classList.remove('open'); drawer.setAttribute('aria-hidden','true'); }
  ...
  try{
    ...
    if(el.bnMenu && el.drawerOpen){
      el.bnMenu.addEventListener('click', ()=>{
        if(el.drawerOpen){ el.drawerOpen.click(); } else { document.getElementById('drawer')?.classList.add('open'); document.getElementById('drawerScrim')?.classList.add('show'); }
      });
    }
...
# ============================ /DEV: FAKE PAYMENT TEST ============================
  function bindTap(id, handler){
    ...
    el.addEventListener('pointerup', function(e){ handler(e); }, {passive:false});
    el.addEventListener('click', function(e){ handler(e); }, {passive:false});
  ...
  bindTap('drSettings', function(){ openDetails('settingsDD'); closeDrawer(); });
  bindTap('drSocials',  function(){ openDetails('socialsDD'); closeDrawer(); });
  bindTap('drTnc',      function(){ openDetails('tncDD'); closeDrawer(); });
  bindTap('drProfile',  function(){ openDetails('profileDD'); closeDrawer(); });
  bindTap('drMsg',      function(){ openDetails('msgDD'); closeDrawer(); });
  bindTap('drLogout',   function(){ try{ document.getElementById('btnLogout')?.click(); }catch(_){} closeDrawer(); });

})();pp.py — Arbexa Profit Finder (Spot Arbitrage Scanner) + Minimal Auth (Gmail+Password)
# ----------------------------------------------------------------------------------------------------------------
# WHAT’S FIXED
# - Single FastAPI app instance (no duplicate `app = FastAPI()` resets).
# - /me returns proper profile data; /opps profile card loads it (and caches to localStorage).
