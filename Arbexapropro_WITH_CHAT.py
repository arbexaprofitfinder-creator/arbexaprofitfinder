...
.chatfab{position:fixed;right:14px;bottom:18px;width:52px;height:52px;border-radius:999px;border:1px solid #23345f;background:#0e1a35;z-index:9999;
  display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 10px 24px rgba(0,0,0,.35)}
...
async function sendMessage(ev){
  ...
  const j = await r.json().catch(()=>({}));
  if(!r.ok || !j.ok){ toast(j.detail||('Send failed ('+r.status+')')); return; }
  ta.value='';
  await loadMessages();
}
