...
<script>
(function(){
  const PROTECT = ['arbexa_token','arbexa_public_id','arbexa_username'];
  function backup(){
    const b = {};
    try{ for (const k of PROTECT) b[k] = localStorage.getItem(k); }catch(_){}
    return b;
  }
  function restore(b){
    try{ for (const k of PROTECT) if(b[k]!==undefined && b[k]!==null) localStorage.setItem(k, b[k]); }catch(_){}
  }
  try{
    const b = backup();
    ...
    if (_remove) {
      localStorage.removeItem = function(key){
        if (PROTECT.includes(String(key))) return;
        try{ return _remove(key); }catch(_){}
      };
    }
    setInterval(()=>{
      const bb = backup();
      for (const k of PROTECT){
        if (!localStorage.getItem(k) && bb[k]) localStorage.setItem(k, bb[k]);
      }
    }, 1500);
  }catch(e){ }
})();
...
<section class="sec grid fade">
  ...
  <div class="box">
    ...
    <div class="socials">
      ...
      <a class="sicon" target="_blank" rel="noopener" href="https://t.me/Arbexaprofitfinderchannel"><img alt="Telegram Channel" src="https://logo.clearbit.com/telegram.org"></a>
      <a class="sicon" target="_blank" rel="noopener" href="https://t.me/Arbexaprofitfinderchat"><img alt="Telegram Chat" src="https://logo.clearbit.com/telegram.org"></a>
